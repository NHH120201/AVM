...
const [audioOverlay, setAudioOverlay] = useState<null | "subtitles" | "tts">(null);
 ...
 const syncAudioToTime = useCallback((t:number) => {
  const aud=audioRef.current; if(!aud||isDraggingClip.current) return;
  const clip=clipAtTimeOnTrack(t,1);
  if(!clip){
   // No audio clip at this time: pause audio.
   aud.pause();
   return;
  }
  const url=`file:///${clip.path.replace(/\\/g,"/")}`;
  const offset=Math.max(0,t-clip.startSec+clip.trimStart);
  if(aud.src!==url){
   if(audioLoadingRef.current) return; audioLoadingRef.current=true; aud.src=url; aud.load();
   const onReady=()=>{audioLoadingRef.current=false;aud.currentTime=offset;aud.volume=volume/100;aud.removeEventListener("canplay",onReady);aud.removeEventListener("error",onErr);};
   const onErr=()=>{audioLoadingRef.current=false;aud.removeEventListener("canplay",onReady);aud.removeEventListener("error",onErr);};
   aud.addEventListener("canplay",onReady);aud.addEventListener("error",onErr);
  } else {aud.currentTime=offset;}
 },[volume]);
 ...
 const tick = useCallback(() => {
  ...
  if(clip&&vid&&!vid.paused){
   ...
   const next=currentTimeRef.current+1/60;
   if(next>=totalDurRef.current){setPlaying(false);setCurrentTime(0);currentTimeRef.current=0;return;}
   currentTimeRef.current=next;setCurrentTime(next);
   const nextClip=clipsRef.current.filter(c=>c.track===0&&c.startSec>=next).sort((a,b)=>a.startSec-b.startSec)[0];
   if(nextClip&&nextClip.startSec<=next+0.05){syncVideoToTime(nextClip.startSec);if(vid)vid.play().catch(()=>{});}
  }
  rafRef.current=requestAnimationFrame(tick);
 },[syncVideoToTime,syncAudioToTime]);

 useEffect(() => {
  const vid=videoRef.current;const aud=audioRef.current;
  if(playing){syncVideoToTime(currentTimeRef.current);syncAudioToTime(currentTimeRef.current);if(vid)vid.play().catch(()=>{});if(aud)aud.play().catch(()=>{});rafRef.current=requestAnimationFrame(tick);}
  else{cancelAnimationFrame(rafRef.current);if(vid)vid.pause();if(aud)aud.pause();}
  return()=>cancelAnimationFrame(rafRef.current);
 },[playing,tick,syncVideoToTime,syncAudioToTime]);
 ...
 const updateText=(id:string,patch:Partial<TextClip>)=>{setTextClips(prev=>prev.map(c=>c.id===id?{...c,...patch}:c));};
  ...
  try {
    ...
    for (const seg of segments) {
      ...
      chunks.forEach((chunk, idx) => {
        let rawStart = seg.start + idx * perChunkDur;
        let rawEnd = idx === chunks.length - 1
          ? seg.end
          : seg.start + (idx + 1) * perChunkDur;

        // Prevent overlaps: push start forward if needed
        let chunkStart = rawStart;
        let chunkEnd = rawEnd;
        if (chunkStart < lastEnd) chunkStart = lastEnd;
        if (chunkEnd <= chunkStart + 0.02) return; // too tiny, skip

        // Visually shrink duration a bit so boxes leave a small gap on the timeline
        const fullDur = chunkEnd - chunkStart;
        const visibleDur = Math.max(0.3, fullDur * 0.85);
    ...
    setTextClips(prev => [...prev.filter(c => !c.subtitle), ...newTextClips]);
...
const handleQwenTts = async () => {
 ...
 const onTextClipMouseDown=(e:React.MouseEvent,id:string)=>{
  if(tool!=="select")return;e.preventDefault();e.stopPropagation();setSelectedTextId(id);setSelectedId(null);setSelectedListId(null);
       ...
       <div style={{flex:1,overflowY:"auto",padding:"14px 16px"}}>
        ...
        {(textClips.length>0||listClips.length>0)&&(
         <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:16}}>
          {textClips.map(tc=>(
           <div key={tc.id} onClick={()=>{setSelectedTextId(tc.id);setSelectedListId(null);setSelectedId(null);}} style={{width:130,cursor:"pointer",display:"flex",flexDirection:"column",gap:5}}>
            <div style={{width:130,height:82,background:selectedTextId===tc.id?"#2e1065":"#1a1b1f",border:`1.5px solid ${selectedTextId===tc.id?"#7c3aed":"#26282e"}`,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
        ...
        {selectedTextId&&(()=>{const tc=textClips.find(c=>c.id===selectedTextId);if(!tc)return null;return(
         <div style={{background:"#1a1b1f",border:"1px solid #26282e",borderRadius:10,padding:14,marginBottom:14,display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
           {TEXT_STYLES.map(s=>(
            <div key={s.id} onClick={()=>updateText(tc.id,{textStyle:s.id})} style={{cursor:"pointer",border:`1.5px solid ${tc.textStyle===s.id?"#7c3aed":"#374151"}`,borderRadius:6,padding:"4px 8px",background:tc.textStyle===s.id?"#2e1065":"#111214",display:"flex",alignItems:"center",justifyContent:"center",minWidth:60,height:32,overflow:"hidden"}}>
             <span style={{fontFamily:tc.fontFamily,fontSize:11,...s.wrapStyle,...s.textStyle,whiteSpace:"nowrap"}}>{s.label}</span>
            </div>
           ))}
          </div>
          ...
          <div style={{display:"flex",gap:8}}>
           <select value={tc.fontFamily} onChange={e=>updateText(tc.id,{fontFamily:e.target.value})} style={{flex:1,background:"#111214",color:"#e2e8f0",border:"1px solid #374151",borderRadius:6,padding:"5px 8px",fontSize:11}}>
            {["Georgia","Impact","Courier New","Trebuchet MS","Arial Black","Verdana"].map(f=><option key={f} value={f}>{f}</option>)}
           </select>
        ...
        {selectedListId&&(()=>{const lc=listClips.find(c=>c.id===selectedListId);if(!lc)return null;return(
         <div style={{background:"#1a1b1f",border:"1px solid #26282e",borderRadius:10,padding:14,marginBottom:14,display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
           {TEXT_STYLES.map(s=>(
            <div key={s.id} onClick={()=>updateList(lc.id,{textStyle:s.id})} style={{cursor:"pointer",border:`1.5px solid ${lc.textStyle===s.id?"#22c55e":"#374151"}`,borderRadius:6,padding:"4px 8px",background:lc.textStyle===s.id?"#0d2e0d":"#111214",display:"flex",alignItems:"center",justifyContent:"center",minWidth:60,height:32,overflow:"hidden"}}>
             <span style={{fontFamily:lc.fontFamily,fontSize:11,...s.wrapStyle,...s.textStyle,whiteSpace:"nowrap"}}>{s.label}</span>
            </div>
           ))}
          </div>
          <div style={{display:"flex",gap:8}}>
           <select value={lc.fontFamily} onChange={e=>updateList(lc.id,{fontFamily:e.target.value})} style={{flex:1,background:"#111214",color:"#e2e8f0",border:"1px solid #374151",borderRadius:6,padding:"5px 8px",fontSize:11}}>
            {["Impact","Georgia","Courier New","Trebuchet MS","Arial Black","Verdana"].map(f=><option key={f} value={f}>{f}</option>)}
           </select>
           <div style={{display:"flex",alignItems:"center",gap:5,background:"#111214",border:"1px solid #374151",borderRadius:6,padding:"0 8px"}}>
    ...
    <div ref={tlRef} onMouseDown={onTimelineMouseDown} style={{flex:1,overflowX:"auto",overflowY:"hidden",position:"relative",cursor:tool==="razor"?"crosshair":"default"}}>
     <div style={{width:tlWidth,position:"relative"}}>
      ...
      <div style={{height:52,borderBottom:"1px solid #0d0f12",position:"relative",background:"#020617"}} onClick={()=>{setSelectedId(null);setSelectedTextId(null);}}>
       {textClips.filter(tc=>tc.subtitle).map(tc=>{const w=Math.max(tc.durationSec*zoom,4);const isSel=tc.id===selectedTextId;return(
        <div id={`tclip-${tc.id}`} key={tc.id} onMouseDown={e=>onTextClipMouseDown(e,tc.id)} style={{position:"absolute",left:tc.startSec*zoom,top:4,width:w,height:44,background:isSel?"#0369a1bb":"#0ea5e9bb",border:`1.5px solid ${isSel?"#e0f2fe":"#38bdf8"}`,borderRadius:6,overflow:"hidden",cursor:"grab",boxSizing:"border-box",display:"flex",alignItems:"center",paddingLeft:8,fontSize:11,color:"#e0f2fe",zIndex:isSel?5:1}}>
         CC {tc.label}
      ...
      <div style={{height:52,borderBottom:"1px solid #0d0f12",position:"relative",background:"#0c0a14"}} onClick={()=>{setSelectedId(null);setSelectedTextId(null);}}>
       {textClips.filter(tc=>!tc.subtitle).map(tc=>{const w=Math.max(tc.durationSec*zoom,4);const isSel=tc.id===selectedTextId;return(
        <div id={`tclip-${tc.id}`} key={tc.id} onMouseDown={e=>onTextClipMouseDown(e,tc.id)} style={{position:"absolute",left:tc.startSec*zoom,top:4,width:w,height:44,background:isSel?"#4c1d95bb":"#2e1065bb",border:`1.5px solid ${isSel?"#a78bfa":"#7c3aed"}`,borderRadius:6,overflow:"hidden",cursor:"grab",boxSizing:"border-box",display:"flex",alignItems:"center",paddingLeft:8,fontSize:11,color:"#ddd6fe",zIndex:isSel?5:1}}>
         T {tc.label}
