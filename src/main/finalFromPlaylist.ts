...
const OUT_ROOT = "C:\\Users\\Admin\\.openclaw\\workspace\\AVM\\Video\\EditVideo\\CutClips";
...
export async function buildFinalFromPlaylist(
  ...
  for (const item of items) {
    ...
    } else if (duration < 2 * secPerClip) {
      // ── RULE 2: between 1× and 2× secPerClip → single 10s segment, goes into pool ───
      fileIndex++;
      const outPath = path.join(tempDir, `clip-${String(fileIndex).padStart(4, "0")}.mp4`);
      console.log(`  [Rule 2] ${duration.toFixed(2)}s → one ${secPerClip}s chunk`);
      await runFfmpegCut(item.path, outPath, 0, secPerClip);
      await runFfmpegNormalize(outPath);
      randomPool.push({ path: outPath, durationSec: secPerClip });

    } else {
      // ── RULE 3: >= 2× secPerClip (e.g. 20s+ when secPerClip=10) → split into N × 10s chunks ──
  ...
  // ─── Assemble finalClips from unified pool ────────────────────────────────────
  // All direct clips first (preserve order), then shuffled Rule-3 chunks.
  const unifiedPool: PoolClip[] = [
    ...directClips,
    ...shuffle(randomPool),
  ];

  let totalSec = 0;
  const finalClips: PoolClip[] = [];
  console.log(
    `[Build] Pool: ${directClips.length} direct + ${randomPool.length} random = ${unifiedPool.length} clips`
  );

  const MIN_LAST_CLIP_SEC = 5;

  for (const clip of unifiedPool) {
    ...
    if (remaining < MIN_LAST_CLIP_SEC) {
      console.log(
        `[Build] Stopping early — only ${remaining.toFixed(
          2
        )}s left, below ${MIN_LAST_CLIP_SEC}s minimum`
      );
      break;
    }

    if (clip.durationSec <= remaining) {
      // Fits entirely
      finalClips.push(clip);
      totalSec += clip.durationSec;
    } else {
      // Overshoots target — trim last segment to fill remaining time exactly
      fileIndex++;
      const trimmedPath = path.join(
        tempDir,
        `clip-${String(fileIndex).padStart(4, "0")}.mp4`
      );
      console.log(
        `[Build] Last chunk: trimming ${clip.durationSec.toFixed(
          2
        )}s → ${remaining.toFixed(2)}s`
      );
      ...
      totalSec += remaining;
      break; // we're exactly at or very close to maxDurationSec
    }

    console.log(
      `[Build] +${clip.durationSec.toFixed(2)}s → ${totalSec.toFixed(
        2
      )}s / ${maxDurationSec}s`
    );
  }
  ...
  console.log(
    `[Build] Final: ${finalClips.length} clips → ~${totalSec.toFixed(2)}s`
  );
  ...
  if (narrationAudioPath && fs.existsSync(narrationAudioPath)) {
    ...
    try {
      await runFfmpegMuxAudio(baseOutputPath, narrationAudioPath, muxedPath, trimToNarration);
