...
const WORKSPACE = "C:\\Users\\Admin\\.openclaw\\workspace-clawboy";
const OUT_ROOT = "C:\\Users\\Admin\\Videos\\animal_pipeline";
const SCRAPER_DIR = path.join(WORKSPACE, "video-tools", "scraper");
const URLS_JSON_PATH = path.join(SCRAPER_DIR, "urls.json");

export type StepId = "scraper" | "editor";
...
export async function runPipeline(opts: PipelineOptions): Promise<{
  ...
  try {
    await runProcess(
      "node",
      [
        "video-tools/scraper/download_animals.js",
        "--topic", opts.topic,
        "--minDuration", "5",
        "--maxDuration", "600",
        "--maxVideos", "10",
    ...
    opts.onStep("scraper", "done");
  } catch (err: any) {
    opts.onLog(`[ERROR] Scraper failed: ${err.message}`);
    opts.onStep("scraper", "error");
